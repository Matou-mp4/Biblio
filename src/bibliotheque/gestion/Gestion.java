...
public class Gestion {
    ...
    private void gestExemplaires() {
        ...
        //fait : attribuer rayon
        int pos = Utilitaire.choixListe(lrayon) - 1;
        ex.setRayon(lrayon.get(pos));
    ...
    private void gestOuvrages() {
        ...
        switch (choix) {
            ...
            case 3:
                ...
                do {
                    ...
                    if (!(((DVD) o).getAutresLangues().contains(langues.get(choix - 1))) && !(o.getLangue().equals(langues.get(choix - 1)))) {
                        ((DVD) o).getAutresLangues().add(langues.get(choix - 1));
                    }
                    //fait : vérifier unicité ou utiliser set et pas de doublon avec langue d'origine
                } while (true);
                System.out.println("sous-titres");
                do {
                    choix = Utilitaire.choixListe(langues);
                    if (choix == langues.size()) break;
                    if (!(((DVD) o).getSousTitres().contains(langues.get(choix - 1)))) {
                        ((DVD) o).getSousTitres().add(langues.get(choix - 1));
                    }
                     //fait  vérifier unicité ou utiliser set
                } while (true);
                ;
                break;
        }
        ...
        //fait ajouter 1 auteur à la liste des auteurs
        gestAuteurs();
    }
